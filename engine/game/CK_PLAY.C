...
/* [web port] debug probe: expose live player/scroll state so the headless
   smoke test can prove in-level movement (Keen's x advances + the view
   scrolls) rather than just a static render. Harmless; KEEPALIVE-exported. */
#include <emscripten.h>
EMSCRIPTEN_KEEPALIVE int CKWEB_PlayerState (int what)
{
	switch (what)
	{
		case 0: return ingame ? 1 : 0;
		case 1: return player ? (int)player->x : -1;
		case 2: return player ? (int)player->y : -1;
		case 3: return (int)originxglobal;
		case 4: return (int)originyglobal;
...
/* [web port] deterministic level-warp for headless verification. Performs the
   exact effect of the engine's built-in "W = warp to level" cheat (CK_PLAY.C
   DebugKeys) — set the target map and raise ex_warped — without the typed
   US_LineInput UI. The running GameLoop picks this up at the next frame
   boundary: PlayLoop exits (playstate != ex_stillplaying), the switch falls
   through, and the do/while re-enters SetupGameLevel() to load the level.
   Only acts while a game is in progress; returns 1 if the warp was queued. */
EMSCRIPTEN_KEEPALIVE int CKWEB_Warp (int level)
{
	if (!ingame || level < 1 || level > 18)
		return 0;
	gamestate.mapon = level;
	playstate = ex_warped;
	return 1;
