...
typedef struct animtilestruct
{
	unsigned	x,y,tile;
	tiletype	*chain;
	unsigned short	far *mapplane;	// [web port] 16-bit map words
...
void RF_MarkTileGraphics (void)
{
	...
	unsigned short	far	*start,far *end,far *info;	// [web port] 16-bit map words
	unsigned	i,tilehigh;
	char		str[80],str2[10];
			...
			if (tinf[ANIM+tile])
			{
				...
				if (tinf[SPEED+tile])
				{
					...
					for (i=0;i<numanimchains;i++)
						...
						{
							*info = i;	// [web port] index, not a truncated 32-bit pointer
							goto nextback;
						}
...
void RFL_CheckForAnimTile (unsigned x, unsigned y)
{
	...
	unsigned short	far *map;	// [web port] 16-bit map words
...
//
	...
	if (tinf[ANIM+tile] && tinf[SPEED+tile])
	{
		...
		anim->tile = tile;
		anim->mapplane = map;
		anim->chain = &allanims[*(mapsegs[2]+offset)];	// [web port] info plane holds an allanims[] index (a 32-bit pointer can't fit the 16-bit info word; the old truncated-pointer cast made RFL_AnimateTiles write garbage tiles back into the map)
#ifdef KEEN6
		anim->chain->visible++;
#endif
	}
				  ...
				  unsigned width, unsigned height)
{
	...
	unsigned short	far *source0, far *source1, far *source2;	// [web port] 16-bit map words
	unsigned short	far *dest0, far *dest1, far *dest2;
	boolean		changed;
...
void RF_MemToMap (unsigned short far *source, unsigned plane,
				  unsigned destx, unsigned desty,
				  unsigned width, unsigned height)
...
//
	...
	/* [web port] clear the update list and terminate it (was rep stosw). */
	memset (newupdate, 0, UPDATESCREENSIZE-2);
	*(word *)(newupdate + (UPDATESCREENSIZE-2)) = UPDATETERMINATE;
