...
{
	/* [web port] Copy the 8x8 char tile (char8) from grsegs[STARTTILE8] into
	   the planar `dest` buffer. Source is 4 planes x 8 bytes (32 B/char);
	   plane p lands at dest + p*planesize, and within a plane each of the 8
	   rows (1 byte wide) is at +r*width. Replaces the segment-register asm. */
	Uint8 *src = (Uint8 *)grsegs[STARTTILE8] + (unsigned)char8 * 32;
	Uint8 *dbase = (Uint8 *)dest;
	int plane, row;

	for (plane = 0; plane < 4; plane++)
	{
		Uint8 *dp = dbase + plane * planesize;
		for (row = 0; row < 8; row++)
		{
			*dp = *src++;
			dp += width;
...
void ShiftScore (void)
{
	...
	CAL_ShiftSprite ((byte far *)dest,dest->sourceoffset[0],
...
*/
	...
	/* [web port] The score box is a fixed-position overlay sprite (priority 3).
	   On a static, non-scrolling screen the DOS code only re-places it when the
	   score/position changes, relying on the dirty-tile refresh to keep it on
	   top. In this port a background tile copied under the box during map
	   load/scroll is never revisited once the screen settles, so it bleeds
	   through the LED display (the green/brown that shows in the score row).
	   Re-place the box every refresh so it is always redrawn opaque over the
	   current page; it is one small sprite, so the extra blit is negligible. */
	changed = true;
