...
static  WindowRec       wr;
	...
	/* [web port] removed `asm sti`: no DOS interrupts to re-enable */
...
{
/* [web port] CK_SAVEDIR routes saves to the persistent /save mount (empty on
   DOS). The slot digit is the 'x' just before ".EXTENSION"; index it from the
   end so the prefix length doesn't matter. */
static  char    name[] = CK_SAVEDIR "SAVEGAMx."EXTENSION;

	name[sizeof(name) - sizeof(EXTENSION) - 2] = '0' + game;
	return(name);
}
...
USL_ReadConfig(void)
{
	...
	if ((file = open(CK_SAVEDIR "CONFIG."EXTENSION,O_BINARY | O_RDONLY)) != -1)
	{
...
USL_WriteConfig(void)
{
	...
	file = open(CK_SAVEDIR "CONFIG."EXTENSION,O_CREAT | O_BINARY | O_WRONLY,
	...
	if (file != -1)
	{
		...
		CKWEB_PersistFS();	/* [web port] flush options+high scores to IndexedDB */
#endif
	}
}
...
/* [web port] The DOS build flushes the config (options + high scores) to disk
   only in US_Shutdown, i.e. on program exit. A browser tab has no such exit, so
   the JS shell calls this when the page is hidden/closed to persist them. Saved
   games are already flushed the moment they're written (USL_DoSaveGame). */
EMSCRIPTEN_KEEPALIVE
void CKWEB_FlushConfig(void)
{
	if (US_Started && !abortprogram)
		USL_WriteConfig();	/* writes /save/CONFIG.* and CKWEB_PersistFS()es it */
}
#endif
...
//
///////////////////////////////////////////////////////////////////////////
/* [WEBPORT] DOS text-mode VRAM (segment 0xB800) has no browser equivalent. The
   signon screen and TED-launch text UI write here; redirect them to a harmless
   scratch buffer so the writes are safe (the cosmetic text screen isn't shown).
   The +8 guard byte covers the off-by-one offsets a couple of call sites use. */
static byte ck_textvram[8 + 80*25*2];
#define CK_TEXTVRAM(off) ((void *)(ck_textvram + 8 + (off)))
...
{
	/* [web port] The DOS startup/shutdown text screen (BIOS mode 3 + cursor
	   positioning via int 10h) has no equivalent in the browser. No-op. */
}
...
US_TextScreen(void)
{
	...
	_fmemcpy(CK_TEXTVRAM(0),7 + &introscn,80 * 25 * 2);
...
USL_Show(word x,word y,word w,boolean show,boolean hilight)
{
	...
	screen = CK_TEXTVRAM(((x - 1) * 2) + (y * 80 * 2));
	oscreen = (&introscn + 7) + ((x - 1) * 2) + (y * 80 * 2) - 1;
...
//	USL_Show(21,9,4,videocard == VGAcard,b);
	...
	if (b && AdLibPresent)  // Hack because of two lines
	{
		...
		y = 16;
		w = 14;
		screen = CK_TEXTVRAM((x * 2) + (y * 80 * 2) - 1);
				...
				int maxchars,int maxwidth)
{
	...
	*olds = '\0';
	...
	while (!done)
	{
		...
		/* [web port] removed `asm pushf/cli ... popf`: input latches are
		   updated on the same thread, no interrupt race to guard against */
		sc = LastScan;
		LastScan = sc_None;
		c = LastASCII;
		LastASCII = key_None;
