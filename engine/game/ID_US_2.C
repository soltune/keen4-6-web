...
#include "ID_HEADS.H"
#include <time.h>		/* [web port] for the CK6 creature-quiz seed (was int21h) */
#pragma hdrstop
...
#endif
		// Web port: QUIT removed - Quit()->exit(0) freezes the browser
		{uii_Bad}
	};
...
USL_DoSaveGame(UserItem far *item)
{
	...
	CKWEB_PersistFS();	/* [web port] flush the saved game (or its removal) to IndexedDB */
#endif
	USL_SetupCard();
}
...
#else
			/* [web port] The pixel-precise blitter draws a sprite at its exact
			   x, so the DOS BALL1PIXELTOTHERIGHTSPR trick (its 5px ball is
			   pre-shifted +1px inside the sprite byte to reach odd pixels under
			   byte-aligned EGA writes) would land the ball at x+1..x+5 — one
			   pixel past the 5px-wide erase below — leaving a dotted trail on
			   every odd frame. BALLSPR alone is already pixel-accurate here, so
			   use it for all x: smooth, correctly positioned, and trail-free. */
			VWB_DrawSprite(x,y,BALLSPR);
#endif
		}
...
USL_SetupCard(void)
{
	...
	BottomS3 = cstackptr? "ESC to back out" : "";
...
USL_UpLevel(void)
{
	if (!cstackptr)
	{
		// Web port: ESC at the top menu used to quit (Quit()->exit(0)),
		// which freezes the browser - so do nothing here.
		return;
	}
...
boolean US_ManualCheck(void)
{
...
#ifdef __EMSCRIPTEN__
	/* [web port] Keen 6's "type the creature's name from the manual" prompt is
	   copy-protection DRM, not gameplay — and the printed manual it referenced
	   doesn't exist in a browser, so it would block the entire episode behind an
	   unanswerable quiz. Auto-pass it (as faithful source ports do) so CK6 is
	   actually playable; every bit of real game content is still present. */
	checkpassed = true;
	return true;
#endif
	...
	if (listindex == -1)
	{
		/* [web port] The DOS original read the wall clock via int 21h
		   (AH=2Ch time -> CH hours; AH=2Ah date -> DL day) purely to vary the
		   quiz creature between sessions. Use portable C time for the same
		   pseudo-random hour+day seed. */
		{
			time_t tnow = time(NULL);
			struct tm *lt = localtime(&tnow);
			x = lt ? lt->tm_hour : 0;	// hours
			y = lt ? lt->tm_mday : 0;	// day
		}
