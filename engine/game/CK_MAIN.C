...
boolean tedlevel;
word tedlevelnum;	/* [web port] was Uint16; must match `extern word tedlevelnum`
					   in ID_US.H (Uint16 != word once int is 32-bit on wasm) */
...
//
	...
	// [WEBPORT] Under flat memory there is no DOS text-screen segment to
	// reclaim; introscn is a plain static buffer, so skip MML_UseSpace (adding
	// its address to the MM free list would corrupt the heap).
	(void)segstart; (void)seglength;
...
void main(void)
{
	...
	// [web port] There is no DOS text-mode signon screen to acknowledge, so
	// skip the "Press a Key" wait in US_FinishTextScreen (and the DOS ordering
	// screen in Quit). NoWait is the engine's own flag for exactly this.
	NoWait = true;
