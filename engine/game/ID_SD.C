...
#include "ID_HEADS.H"
#endif
#include "opl2.h"		// [web port] embedded OPL2 FM synth (replaces the card)
#pragma	hdrstop
#pragma	warn	-pia
...
// [web port] selreg/writereg are unused (alOut goes straight to OPL2_Write);
// readstat is served by the synth's detection-status emulation.
#define	selreg(n)	outportb(0x388,n)
#define	writereg(n)	outportb(0x389,n)
#define	readstat()	OPL2_Status()
...
//	Global variables
	...
	longword	TimeCount;
	word		HackCount;
	/* [web port] Points into audiosegs[] (an array of real, flat chunk
	   pointers). On DOS this aliased the array as packed 16-bit segment values
	   indexed as word[] and resolved each via MK_FP; on a flat address space
	   audiosegs holds true pointers, so SoundTable is a pointer-to-chunk-pointer
	   dereferenced directly. (MK_FP(seg<<4) masks to 16 bits / <1 MB and so
	   destroys a real heap address — that is why every sound effect was silent
	   while music, which passes audiosegs[] straight to SD_StartMusic, worked.) */
	byte		_seg **SoundTable;
	boolean		ssIsTandy;
	word		ssPort = 2;
...
/* [web port] sqHack/sqHackPtr walk the 16-bit IMF music stream; they must be
   exact 16-bit pointers (NOT `word`, which is 32-bit on wasm32) or the per-event
   stride is wrong and the song desyncs into silence. Lengths stay `word`. */
static	uint16_t		*sqHack,*sqHackPtr;
static	word			sqHackLen,sqHackSeqLen;
...
SDL_PCService(void)
{
	...
	if (pcSound)
	{
		...
		{
			pcLastSample = s;
			Speaker_SetFreq(s ? 1193180.0 / pcSoundLookup[s] : 0.0);
		}
...
alOut(byte n,byte b)
{
	/* [web port] the AdLib card register write becomes a write to the
	   embedded OPL2 synth; the original asm port I/O + timing delays are
...
#else
	/* [web port] AdLib-detection settle delay not needed */
#endif
...
//
///////////////////////////////////////////////////////////////////////////
/* [web port] The wall-clock pump (ck_web.c) owns TimeCount — it must tick even
   before the audio context is unlocked. This audio-driven service therefore must
   NOT also advance TimeCount, or the two stack: the game then runs fast by a
   browser-dependent amount (the Web Audio render rate, hence how fast this ISR
   fires relative to real time, varies per browser). Music/SFX (alTimeCount and
   the OPL services below) stay audio-driven; only the game clock is gated. */
extern int ckweb_realtime;
...
asm	out	dx,al
	...
	if (MusicMode == smm_AdLib)
	{
		...
		{
			LocalTime++;
			if (!ckweb_realtime) TimeCount++;	// [web port] wall clock owns TimeCount
			if (SoundUserHook)
				SoundUserHook();
		}
		if (!(count & 3))
	...
	/* [web port] DOS timer-chain + 8259 IRQ-ack removed: the host has no
	   prior timer ISR to forward to and no PIC to acknowledge. */
...
SD_SetSoundMode(SDMode mode)
{
	...
	if (result && (mode != SoundMode))
	{
		...
		SoundMode = mode;
#ifndef	_MUSE_
		SoundTable = &audiosegs[tableoffset];	// [web port] real chunk pointers
#endif
		SDL_StartDevice();
	}
...
SD_Startup(void)
{
	...
	OPL2_Init(44100.0);		// [web port] init the embedded FM synth; the JS
							// audio shell may re-init with the real context rate
...
SD_PlaySound(soundnames sound)
{
	...
	// [web port] audiosegs holds real flat pointers: take the chunk pointer
	// directly instead of the DOS segment round-trip through MK_FP (which would
	// truncate the address to <1 MB and yield garbage -> silent sound effects).
	s = (SoundCommon far *)SoundTable[sound];
	// [web port] Be tolerant of an absent/zero-length sound chunk: the DOS build
	// aborted via Quit(), but on the web a missing sound must NOT kill the game
	// (and Quit's shutdown path can't run cleanly mid-frame). Just skip it.
	if (!s || !s->length)
		return;
...
{
	/* [web port] On DOS the timer-0 ISR cleared the sound flag once an effect
	   finished; here that stepping happens inside SD_AudioRender (the JS audio
	   sink). When nothing is pulling audio — headless tools, before the browser
	   audio context is unlocked, or while the single main thread is parked in
	   this very loop — the flag would never clear and the original busy-wait
	   wedged the cooperative (ASYNCIFY) event loop forever. Yield so the audio
	   sink can run and finish the sound, capped on the wall clock so we always
	   make progress even with no sink at all. */
	longword deadline = TimeCount + 3 * TickBase;	/* ~3 s safety net */
	while (SD_SoundPlaying() && TimeCount < deadline)
		CKWEB_Yield();
}
...
//
//	[web port] Audio integration entry points.
//
//	On DOS the timer-0 ISR (SDL_t0Service) fired at SDL_SetIntsPerSec() Hz and
//	did everything: stepped music/SFX (writing OPL registers) and advanced
//	TimeCount. Here the JS audio sink pulls PCM via SD_AudioRender(); we run the
//	service at the engine-programmed interrupt rate while rendering, so music,
//	sound effects and TimeCount all advance in lock-step with the audio clock —
//	exactly as the original interrupt did.
//

void SD_ServiceTick(void)		// call once to advance the sound service a tick
{
	SDL_t0Service();
}
...
#endif
void SD_AudioRender(float *buf, int len)	// fill len mono samples
{
	static double tickacc = 0.0;
	double sr = OPL2_SampleRate();
	double intrate = TimerDivisor ? (1192030.0 / (double)TimerDivisor)
								   : (double)(TickBase * 8);
	double samplesPerTick = (intrate > 0.0) ? sr / intrate : sr;
	int pos = 0, i;
	...
	while (pos < len)
	{
		...
		if (tickacc < 1.0)
		{
			SDL_t0Service();			// step music + SFX + TimeCount
			tickacc += samplesPerTick;
		}
		chunk = (int)tickacc;
		if (chunk > len - pos)
			chunk = len - pos;
		if (chunk < 1)
			chunk = 1;
		OPL2_Render(buf + pos, chunk);	// FM + PC-speaker -> PCM
		pos += chunk;
		tickacc -= chunk;
	}
}
