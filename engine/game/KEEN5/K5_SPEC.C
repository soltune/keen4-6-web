...
{
	//
	// [WEBPORT] C port of the EGA galaxy starfield. The GALAXY chunk holds four
	// 4000-entry Uint16 arrays: posx[0..3999], velx[4000..7999],
	// posy[8000..11999], vely[12000..15999]. Caller has set write mode 2 +
	// mapmask 15, so each plotted star is colour 15 (white): set the masked bit
	// in all four planes.
	//
	Uint16 far *g = (Uint16 far *)grsegs[GALAXY];
	...
	//
	// clear the destination page (0x1000 words across all four planes)
	//
	for (i = 0; i < 0x2000; i++)
	{
		Uint16 o = (Uint16)(dest + i);
		p0[o] = 0; p1[o] = 0; p2[o] = 0; p3[o] = 0;
	}

	for (si = 7998; si >= 0; si -= 2)
	{
		Uint16 k = (Uint16)si >> 1;
		Uint16 px, py, off;
		Uint8 m;

		px = (Uint16)(g[k] + g[k + 4000]);		// posx + velx
		if (px > 40960)					// off the right edge
			continue;
		g[k] = px;
		py = (Uint16)(g[k + 8000] + g[k + 12000]);	// posy + vely
		if (py > 25600)					// off the bottom edge
			continue;
		g[k + 8000] = py;

		off = (Uint16)(ylookup[py >> 7] + dest + (px >> 10));
		m = plotpixels[(px >> 7) & 7];
		p0[off] |= m; p1[off] |= m; p2[off] |= m; p3[off] |= m;
	}
}
...
{
	//
	// [WEBPORT] The DOS code poked the EGA CRTC start-address registers to flip
	// the displayed page. VW_SetScreen sets the same displayed-page offset.
	//
	VW_SetScreen(addr, 0);
}
