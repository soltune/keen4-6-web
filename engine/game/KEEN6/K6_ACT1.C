...
void T_Satellite(objtype *ob)
{
	...
	if (xtry == 0 && ytry == 0)
	{
		...
		//
		// [web port] temp3 is now Sint32 (it may hold a sprite pointer for other
		// actors); mask back to 16 bits so this packed low/high-byte shuffle keeps
		// the exact wrap behaviour it had as a Sint16.
		ob->temp3 = (ob->temp3 << 8) & 0xFFFF;
